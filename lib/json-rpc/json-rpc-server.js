...
module.exports = ({ public = {}, private = {}, onresponse, oncall } = {}) =>
{
	...
	return {
		...
		{
			const errorResponse = this[PRIVATE.MTD_VALIDATE_MESSAGE](message);

			// Throw any error, except for "method not found" errors when there is a oncall handler
			if (errorResponse && (!this.oncall || errorResponse.error.code !== -32601))
			{
				this[PRIVATE.MTD_RESPOND](errorResponse);
				return;
			}
			...
			try
			{
				const callUserMethodHandler = (method, args) =>
				{
					if (errorResponse)
					{
						throw errorResponse;
					}
					return Array.isArray(args)     ? this[PRIVATE.ATT_PUBLIC_PROTOTYPE][method].apply(this[PRIVATE.ATT_DEFAULT_CONTEXT], args)
				...
				const resultOrPromise = this.oncall
					? this.oncall.call(this[PRIVATE.ATT_DEFAULT_CONTEXT], method, args, callUserMethodHandler)
					: callUserMethodHandler(method, args);
