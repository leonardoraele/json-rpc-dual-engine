...
module.exports = ({ registry = {}, onresponse } = {}) =>
({
	__registry: registry,
	onresponse,
	...
	async accept(message)
	{
		...
		try
		{
			const resultOrPromise = Array.isArray(args)			? this.__registry[method](...args)
								  : typeof args === 'object'	? this.__registry[method](args)
																: this.__registry[method]();
			const result = (await Promise.resolve(resultOrPromise)) ?? null;
		...
		else if (typeof id === 'number' && id !== 0 && id % Math.floor(id) !== 0)
		{
			return {
				...
				error:
				{
					code: -32600,
					message: 'Server received an invalid JSON-RPC-2.0 Request object: '
						+ 'If property "id" is present and is a number, it should not contain fractorial parts; '
