...
module.exports = ({ properties, registry = {}, onresponse } = {}) =>
({
	__registry: registry,
	__contexts: {},
	__defaultContext: null,
	__globalProperties: typeof properties === 'object'   ? Object.entries(properties) // Shallow-clones the properties object
	                                                           .reduce((result, [key, value]) => (result[key] = value, result), {})
	                  : typeof properties === 'function' ? Object.entries(properties())
	                                                           .reduce((result, [key, value]) => (result[key] = value, result), {})
	                  : properties === undefined         ? {}
	                  : (() =>
	                  {
	                      throw new Error('Failed to build JsonRpcServer. Cause: Invalid value for option "properties".');
	                  })(),
	onresponse,
	...
	getProperty(key)
	{
		return this.__globalProperties[key];
	},
	createContext(description)
	{
		const symbol = Symbol(description);
		this.__contexts[symbol] = Object.create(this.__globalProperties);
		return symbol;
	},
	setContextProperty(context, key, value)
	{
		this.__contexts[context][key] = value;
	},
	getContextProperty(context, key)
	{
		return this.__contexts[context][key];
	},
	/**
	 * @param {string|JsonRpcRequest} message
	 * @returns
	 */
	async accept(message, context = undefined)
	{
		...
		if (this.__defaultContext === null)
		{
			this.__defaultContext = this.__contexts[this.createContext('default')];
		}

		// Use default context if context for this request was not defined
		const actualContext = context === undefined       ? this.__defaultContext
		                    : typeof context === 'symbol' ? this.__contexts[context]
		                                                  : context;
